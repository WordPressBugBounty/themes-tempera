...
function tempera_title_and_description() {
	...
	if ( is_singular() && has_post_thumbnail( $post->ID ) && 
		...
		$image[1] >= ( $temperas['tempera_sidebar'] + $temperas['tempera_sidewidth'] ) ) {
			$himgsrc = esc_url( $image[0] );
	};

	if ( isset( $himgsrc ) && ( $himgsrc != '' ) ) { echo '<img id="bg_image" alt="' . esc_html( get_bloginfo( 'name' ) ) . '" title="' . esc_html( get_bloginfo( 'name' ) )  . '" src="' . esc_url( $himgsrc ) . '"  />'; } ?>
	...
	switch ($temperas['tempera_siteheader']) {
		case 'Site Title and Description':
			...
			echo '<span> <a href="' . esc_url ( home_url( '/' ) ) . '" title="' . esc_attr( get_bloginfo( 'name', 'display' ) ) . '" rel="home">' . esc_html( get_bloginfo( 'name' ) )  . '</a> </span>';
			echo '</'.$heading_tag.'>';
...
if ( ! function_exists( 'tempera_skiplink' ) ):
function tempera_skiplink() {
	?>
	<a class="skip-link screen-reader-text" href="#main" title="<?php esc_attr_e( 'Skip to content', 'tempera' ); ?>"> <?php _e( 'Skip to content', 'tempera' ); ?> </a>
	<?php
}// tempera_skiplink()
endif;

add_action( 'cryout_body_hook', 'tempera_skiplink', 2 );
	...
	for ($i = 1; $i <= 9; $i+=2) {
		...
		if ( ${"tempera_social$j"} ) {
			...
			class="socialicons social-<?php echo esc_attr(${"tempera_social$i"}); ?>" title="<?php echo ${"tempera_social_title$i"} !="" ? esc_attr(${"tempera_social_title$i"}) : esc_attr(${"tempera_social$i"}); ?>">
				<img alt="<?php echo esc_attr(${"tempera_social$i"}); ?>" src="<?php echo esc_url( get_template_directory_uri() ) . '/images/socials/' . ${"tempera_social$i"} . '.png'; ?>" />
			</a><?php
		}
	}
	echo '</div>';
...
if ( $temperas['tempera_backtop']=="Enable" ) add_action( 'cryout_footer_hook', 'tempera_back_top' );
...
function tempera_breadcrumbs() {
	...
	if ( is_front_page() && is_home() ) {
		...
		if ($temperas['tempera_frontpage']=="Enable") { return; }
		...
		echo '<div class="breadcrumbs">' . $home . $separator .  get_the_title( get_option('page_for_posts', true) ) . '</div>';

...
= 1.8.1 =
* Added back-to-top button support for mobile devices by placing it in the footer
* Improvided inner-content padding/margin/border styling support for custom post types 
* Fixed a typo in the style enqeueues possibly breaking child theme support since 1.8.0
* Fixed menu font not working due to a typo since 1.8.0
* Rewrote handling of 'tempera_totalSize' variable to avoid a PHP undefined warning in some edge cases

= 1.8.0 =
* Rewrote styles enqueues ***will require changes in the child theme for correct handling of styles
* Rewrote some presentation page code (includes/theme-frontpage.php is no longer used) ***may require adaptations in child themes that customize the presentation page functionality
